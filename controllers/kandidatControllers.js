...
// Create Kandidat dpp
const createKandidatDPP = asyncHandler(async (req, res) => {
  const userId = req.user._id;
  const nbi = req.body.nbi;
  const findKandidat = await Kandidat.findOne({ nbi: nbi });
  const findDeadline = await Kandidat.findOne({
  ...
  if (!findKandidat && req.body.kategori == "DPP") {
    const newKandidat = await Kandidat.create({
      ...req.body,
      createdByAdmin: userId,
    });
    ...
    const terdaftar = findKandidat.terdaftar;
    if (terdaftar !== true && req.body.kategori === "DPP") {
      const updateKandidat = await Kandidat.findByIdAndUpdate(
        findKandidat._id,
        {
          terdaftar: true,
      ...
      res.json(updateKandidat);
    } else if (terdaftar !== true || req.body.kategori !== "DPP") {
      res.status(403).json({ message: "Tidak memiliki izin atau hak akses" });
    } else {
      res.status(400).json({ message: "Kandidat Sudah Terdaftar" });
    }
...
// Get All Kandidat DPP
const getAllKandidatDPP = asyncHandler(async (req, res) => {
  try {
    const getKandidat = await Kandidat.find({
      kategori: "DPP",
      terdaftar: true,
...
// Update a Kandidat DPP
const updateAKandidatDPP = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const userId = req.user.id;
  validateMongoDBid(id);
  const kandidat = await Kandidat.findById(id);
  if (kandidat.kategori === "DPP") {
    try {
      const updateKandidat = await Kandidat.findByIdAndUpdate(
        id,
        {
...
//Delete a Kandidat DPP
const deleteAKandidatDPP = asyncHandler(async (req, res) => {
  const { id } = req.params;
  validateMongoDBid(id);
  const kandidat = await Kandidat.findById(id);
  if (kandidat.kategori === "DPP") {
...
// Set deadline voting DPP
const setDeadlineVoteDPP = asyncHandler(async (req, res) => {
  const deadlineValue = req.body.votingDeadline;
  const findKandidat = await Kandidat.findOne({
    kategori: "DPP",
    terdaftar: true,
...
// Set Start Vote DPP
const setStartVoteDPP = asyncHandler(async (req, res) => {
